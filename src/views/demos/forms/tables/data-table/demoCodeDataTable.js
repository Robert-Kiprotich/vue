...
const headers = [
  { title: 'Date', key: 'full_name' },
  { title: 'Text', key: 'email' },
  { title: 'From', key: 'start_date' },
  { title: 'To', key: 'salary' },
  { title: 'Cost', key: 'age' },
...
const resolveStatusVariant = (status: number) => {
  if (status === 1)
    ...
    return { color: 'success', text: 'Failed' }
  else if (status === 3)
    return { color: 'error', text: 'Rejected' }
  else if (status === 4)
    return { color: 'warning', text: 'User on Blacklist' }
  else
    return { color: 'info', text: 'Success' }
}
</script>
